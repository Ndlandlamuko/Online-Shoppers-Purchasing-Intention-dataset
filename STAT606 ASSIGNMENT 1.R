...
preds_dt_test <- h2o.predict(dt_model, test_h2o)
...
train_dt_pred <- cbind(train_set,
                       setNames(preds_dt_train[, 3, drop = FALSE], "predicted_prob")) 

test_dt_pred <- cbind(test_set, 
                      setNames(preds_dt_test[, 3, drop = FALSE], "predicted_prob"))
...
# Determine the predicted classlabels:

train_dt_pred$pred_class <- factor(
  ifelse(train_dt_pred$predicted_prob > threshold, "TRUE", "FALSE"),
  levels = levels(train_dt_pred[[target]])
) # training set


test_dt_pred$pred_class <- factor(
  ifelse(test_dt_pred$predicted_prob > threshold, "TRUE", "FALSE"),
...
roc_dt_train <- roc(train_dt_pred[[target]], train_dt_pred$predicted_prob)
auc(roc_dt_train)
plot(roc_dt_train)
...
roc_dt_test <- roc(test_dt_pred[[target]], test_dt_pred$predicted_prob)
auc(roc_dt_test)
plot(roc_dt_test)
